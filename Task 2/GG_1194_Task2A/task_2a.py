...
def detect_ArUco_details(image):
    ...
    * ArUco_corners = 
    ...
    ##############	ADD YOUR CODE HERE	##############
    # This is predefine dicitonary for aruco ids and size for reference 
    dict = aruco.getPredefinedDictionary(aruco.DICT_4X4_250)
    # marker var use detectorParameters which set all the parameters of detected aruco
    marker = aruco.DetectorParameters()
    # Changing image to gray for better detection and less computation
    gray_scale = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    # This is the main function in aruco lib as it detect and mark all the corners and ids according to dict and params
    marker_corners , marker_IDs, reject = aruco.detectMarkers(gray_scale, dict, parameters = marker)

    # We are checking if any ids are present in the image or not for error/exception handleing
    if marker_IDs is not None:
        # As we have multiple aruco marker in one image we have to itrate over every ids to find particular values
        for i in range(len(marker_IDs)):
            # We need ids to be in int so parsing it to int using py
            marker_IDs1 = int(marker_IDs[i][0])

            # using corners dict for putting corners values with key = ids and value = x,y cordinates of corners
            ArUco_corners[marker_IDs1] = marker_corners[i][0]
            
            # Findint the center using all four corners and taking particular x and y coordinates
            centerX_coor = int((marker_corners[i][0][0][0] + marker_corners[i][0][2][0]) / 2)
            centerY_coor = int((marker_corners[i][0][0][1] + marker_corners[i][0][2][1]) / 2)

            # Finding the angle of the aruco with respect to vertical axis using math lib to calculate func.
            x1, y1, x2, y2 = marker_corners[i][0][0][0], marker_corners[i][0][0][1], marker_corners[i][0][1][0], marker_corners[i][0][1][1]
            angle = int(math.degrees(math.atan2(y2 - y1, x2 - x1)))
            # We are given particualr value to use according to particualr quadrant so making it accordinly.
            angle *= -1

            # All the details of center coordinates and angle are used as value in the dict with ids as keys.
            ArUco_details_dict[marker_IDs1] = [[centerX_coor, centerY_coor], angle]
